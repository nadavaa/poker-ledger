...
1a. **Withdrawing after buying in.** A player with a live buy-in cannot withdraw themselves: their money is in the pot and the settlement maths is built on the roster. Available while `scheduled`, and while `active` only if they have no non-voided buy-in — they signed up and never showed. Blocked once counting starts. Enforced by a *restrictive* update policy on `game_signups` whose with-check only bites on a withdrawal, so promotions and ordinary signup edits are unaffected. The admin path is unchanged: removing a player voids their buy-ins, which is a deliberate act with an audit trail. The screen replaces the button with the reason rather than showing a dead disabled control.
